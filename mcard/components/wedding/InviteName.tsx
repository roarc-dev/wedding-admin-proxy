...
function renderInvitationSegments(
    text: string,
    pretendardSemiBoldStyle?: React.CSSProperties
): JSX.Element[] {
    const out: JSX.Element[] = []
...
export default function InviteName(props: InviteNameProps) {
    const { pageId = "default", style } = props

    const [weddingData, setWeddingData] = useState<WeddingData>({
        invitationText: `저희 두 사람이 하나 되는 약속의 시간에 마음을 담아 소중한 분들을 모십니다.\n귀한 걸음으로 축복해 주시면 감사하겠습니다.`,
        groomFatherName: "박태식",
        groomMotherName: "김정아",
        groomName: "민준",
        brideFatherName: "최성재",
    ...
    const pretendardRegularStyle: React.CSSProperties = useMemo(() => {
        return {
    ...
    const pretendardSemiBoldStyle: React.CSSProperties = useMemo(() => {
        return {
            fontFamily: pretendardFontFamily,
            fontWeight: 600,
        }

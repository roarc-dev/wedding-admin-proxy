...
type CommentsApiResponse =
    | { success: true; data: CommentItem[]; count?: number }
...
export default function CommentBoard({
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            const msg = error instanceof Error ? error.message : '알 수 없는 오류'
            setErrorMessage(`등록 중 오류가 발생했습니다: ${msg}`)
        }
    }
    ...
    async function confirmDelete() {
        ...
        try {
            ...
            if (result.success) {
                ...
                throw new Error(
                    result.error || '삭제 중 오류가 발생했습니다. 다시 시도해주세요.'
                )
            }
        } catch (error: unknown) {
            console.error('댓글 삭제 에러:', error)
            const msg = error instanceof Error ? error.message : '알 수 없는 오류'
            setErrorMessage(`삭제 중 오류가 발생했습니다: ${msg}`)
        }
    }
            ...
            try {
                ...
                const serverComments =
                    typeof json === 'object' && json !== null
                        ? ((json as { data?: { comments?: unknown } }).data?.comments ??
                          (json as { comments?: unknown }).comments)
                        : undefined
                if (
                    !cancelled &&
                    (serverComments === 'on' || serverComments === 'off')
                ) {

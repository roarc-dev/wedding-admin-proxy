...
interface ItemCardProps {
    ...
    itemsLength: number
    renderLeftControls?: (index: number) => React.ReactNode
}
    ...
    return (
        ...
        >
            ...
            <MarkdownEditor
                ...
                theme={theme}
                leftControls={
                    renderLeftControls ? renderLeftControls(index) : undefined
                }
            />
        </div>
    )
        ...
        <div style={{ width: "100%" }}>
            ...
            <div
                style={{
                    width: "100%",
                    display: "flex",
                    justifyContent: leftControls ? "space-between" : "flex-end",
                    gap: leftControls ? 12 : 4,
                    alignItems: "center",
                }}
            >
                {leftControls ? (
...
// 커스텀 색상 선택기 컴포넌트 (vanilla-colorful 기반 - 카카오톡 인앱 브라우저 호환)
...
// vanilla-colorful 스크립트 로드 상태를 추적하는 전역 플래그
let vanillaColorfulLoaded = false
let vanillaColorfulLoading = false

const loadVanillaColorful = (): Promise<void> => {
    return new Promise((resolve) => {
        if (vanillaColorfulLoaded) {
            resolve()
            return
        }
        if (vanillaColorfulLoading) {
        ...
        vanillaColorfulLoading = true

        const script = document.createElement("script")
        script.type = "module"
        script.innerHTML = `
            import 'https://cdn.skypack.dev/vanilla-colorful/hex-color-picker.js';
        `
        script.onload = () => {
            vanillaColorfulLoaded = true
            resolve()
        }
        // module 스크립트는 onload가 바로 호출되지 않을 수 있으므로 타이머로도 체크
        setTimeout(() => {
            vanillaColorfulLoaded = true
            resolve()
        }, 500)
...
const CustomColorPicker: React.FC<CustomColorPickerProps> = ({
    ...
    const colorPickerRef = useRef<HTMLElement | null>(null)

    // 최신 onChange 참조를 유지하기 위한 ref
    const onChangeRef = useRef(onChange)
    onChangeRef.current = onChange

    // vanilla-colorful 스크립트 로드
    useEffect(() => {
        loadVanillaColorful().then(() => {
            setPickerReady(true)
        })
    }, [])
    ...
    // hex-color-picker 요소 생성 및 이벤트 연결
    useEffect(() => {
        if (!isOpen || !pickerReady || !pickerContainerRef.current) return
        ...
        // hex-color-picker 요소 생성
        const pickerElement = document.createElement("hex-color-picker")
        pickerElement.setAttribute("color", value)
        pickerElement.style.width = "100%"
        pickerElement.style.maxWidth = "200px"

        colorPickerRef.current = pickerElement

        // 이벤트 리스너 추가 (ref를 통해 최신 onChange 참조)
        const handleColorChanged = (event: Event) => {
            const customEvent = event as CustomEvent<{ value: string }>
            const newColor = customEvent.detail.value
            setHexValue(newColor)
            onChangeRef.current(newColor)
        }

        pickerElement.addEventListener("color-changed", handleColorChanged)
    ...
    }, [isOpen, pickerReady, value]) // value를 초기값으로만 사용

    // hexValue가 외부에서 변경되면 picker에도 반영 (빠른 선택 등)
    useEffect(() => {
        if (colorPickerRef.current && isOpen) {
            const currentColor = colorPickerRef.current.getAttribute("color")
            if (currentColor?.toLowerCase() !== hexValue.toLowerCase()) {
                colorPickerRef.current.setAttribute("color", hexValue)
            }
        }
    }, [hexValue, isOpen])
    ...
    // RGB 입력 핸들러
    const handleRgbChange = (type: "r" | "g" | "b", inputValue: number) => {
        const rgb = hexToRgb(hexValue) || { r: 0, g: 0, b: 0 }
        const newRgb = {
            ...rgb,
            [type]: Math.max(0, Math.min(255, inputValue)),
        }
    ...
    return (
        <div ref={pickerRef} style={{ position: "relative", width: "100%" }}>
            ...
            {isOpen && (
                ...
                >
                    {/* vanilla-colorful 컬러 피커 (카카오톡 인앱 브라우저 호환) */}
    ...
    const handleReplaceClick = () => {
        ...
        const handleFileSelect = async (e: Event) => {
            ...
            try {
                ...
                // 정리 작업은 finally 블록에서 수행하여 성공/실패 여부와 관계없이 실행되도록 함
                if (
                    typeof document !== "undefined" &&
                    document.body.contains(input)
                ) {
...
const theme: any = {
    ...
    font: {
        ...
        display: FONT_STACKS.p22 || "P22LateNovemberW01-Regular Regular, serif",
    },
...
async function authenticateAdmin(
    ...
    try {
        ...
        if (result.success) {
            ...
            return {
                success: false,
                error: "아이디와 비밀번호를 확인해주세요. 비밀번호 변경은 카카오톡 채널 [로아크]로 문의해주세요.",
            }
        }
    } catch (error: any) {
        ...
        return {
            success: false,
            error: `로그인에 실패했습니다. 카카오톡 채널 [로아크]로 문의해주세요.]: ${error.message}`,
        }
    }
}
...
/**
 * Cloudflare Worker를 통한 이미지 업로드 (안정화 버전)
 * - 10MB 초과 파일 자동 압축 (Worker 안정성 향상)
 * - 재시도 로직 (최대 3회)
 * - 타임아웃 설정 (60초)
 * - XMLHttpRequest를 사용하여 실제 업로드 진행률 지원
 */
async function uploadToWorker(
    ...
    // 10MB 초과 파일은 업로드 전 클라이언트 측 압축 (Worker 안정성 향상)
    let processedFile = file
    const originalSizeMB = (file.size / 1024 / 1024).toFixed(1)

    if (file.size > WORKER_MAX_SIZE) {
        console.log(
            `[Worker] 파일 크기 ${originalSizeMB}MB > 10MB, 클라이언트 측 압축 시작...`
        )
        onProgress?.(2) // 압축 준비
        try {
            // 압축 진행 상태를 콘솔에 출력 (UI는 별도 처리)
            processedFile = await compressImageClient(
                file,
                WORKER_MAX_SIZE,
                (status) => {
                    console.log(`[Worker] 압축 상태: ${status}`)
                }
            )
            const compressedSizeMB = (processedFile.size / 1024 / 1024).toFixed(
                2
            )
            console.log(
                `[Worker] 압축 성공: ${originalSizeMB}MB → ${compressedSizeMB}MB`
            )
            onProgress?.(15) // 압축 완료, 업로드 시작
        } catch (compressError) {
            console.error("[Worker] 클라이언트 측 압축 실패:", compressError)
            const errorMessage =
                compressError instanceof Error
                    ? compressError.message
                    : `사진 파일이 너무 큽니다. 5MB 이하를 권장합니다.`
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
            console.log(
                `[Worker] 업로드 시도 ${attempt}/${MAX_RETRIES} - ${processedFile.name} (${(processedFile.size / 1024 / 1024).toFixed(2)}MB)`
            )
            ...
            const result = await new Promise<{
                ...
                xhr.upload.onprogress = (event) => {
                    if (event.lengthComputable) {
                        // 압축 후 업로드이므로 15~95% 구간 사용
                        const uploadProgress = Math.round(
                            (event.loaded / event.total) * 80
                        )
                        onProgress?.(15 + uploadProgress)
                    }
                }
                ...
                xhr.onload = () => {
                    if (xhr.status >= 200 && xhr.status < 300) {
                        try {
                            ...
                            reject(new Error("Cloudflare 응답 파싱 실패"))
                        }
                    } else {
                        // HTTP 에러
                        if (xhr.status === 413) {
                            reject(
                                new Error(
                                    `파일이 너무 큽니다 (최대 10MB). 현재: ${(processedFile.size / 1024 / 1024).toFixed(1)}MB`
                                )
                            )
                        } else if (xhr.status === 415) {
                            reject(
                                new Error(
                                    "지원하지 않는 이미지 형식입니다. JPEG, PNG, WebP, AVIF만 지원됩니다."
                                )
                            )
                        } else {
                            reject(
                ...
                xhr.ontimeout = () => {
                    reject(
                        new Error(
                            `업로드 시간 초과 (${TIMEOUT_MS / 1000}초). 네트워크 상태를 확인해주세요.`
                        )
                    )
                }

                xhr.open("POST", "https://upload.roarc.kr/upload")
                xhr.send(formData)
            })
            ...
            if (!result?.ok || !result?.url) {
                throw new Error(
                    result?.error ||
                        "Cloudflare 업로드에서 URL을 받지 못했습니다."
                )
            }

            console.log(`[Worker] 업로드 성공: ${result.url}`)
            ...
            const isServerError =
                lastError.message.includes("서버 연결 실패") ||
                lastError.message.includes("503") ||
                lastError.message.includes("500")
            const baseWait = isServerError ? 2000 : 1000
            const waitTime = Math.min(baseWait * Math.pow(2, attempt - 1), 8000)
            console.log(
                `[Worker] 재시도 대기 ${waitTime}ms... (${attempt}/${MAX_RETRIES})`
            )
            onProgress?.(10) // 재시도 시 진행률 리셋
            await new Promise((resolve) => setTimeout(resolve, waitTime))
        }
    }

    // 최종 에러 메시지 개선
    if (lastError?.message.includes("서버 연결 실패")) {
        throw new Error(
            "이미지 업로드 서버에 연결할 수 없습니다. 네트워크 상태를 확인하거나 잠시 후 다시 시도해주세요."
        )
    }
...
// 이미지 압축 관련 함수들 추가
// Worker 최대 허용 크기: 10MB (안정성 향상을 위해 낮춤, Cloudflare Images CPU time 초과 방지)
const WORKER_MAX_SIZE = 10 * 1024 * 1024
...
// Pica 라이브러리 동적 로드 (고품질 Lanczos 리사이징)
let picaInstance: any = null
let picaLoadPromise: Promise<any> | null = null

async function loadPica(): Promise<any> {
    if (picaInstance) return picaInstance
    if (picaLoadPromise) return picaLoadPromise

    picaLoadPromise = new Promise((resolve) => {
        // 이미 로드되어 있는지 확인
        if (typeof window !== "undefined" && (window as any).pica) {
            picaInstance = new (window as any).pica()
            console.log("[Pica] 기존 인스턴스 사용")
            resolve(picaInstance)
            return
        }

        // 스크립트 태그로 동적 로드
        const script = document.createElement("script")
        script.src = "https://cdn.jsdelivr.net/npm/pica@9/dist/pica.min.js"
        script.async = true

        script.onload = () => {
            if ((window as any).pica) {
                picaInstance = new (window as any).pica()
                console.log("[Pica] 라이브러리 로드 완료")
                resolve(picaInstance)
            } else {
                console.warn("[Pica] 로드됐지만 pica 객체 없음")
                resolve(null)
            }
        }

        script.onerror = () => {
            console.warn("[Pica] CDN 로드 실패, Canvas API로 폴백")
            resolve(null)
        }
    ...
    return picaLoadPromise
}

/**
 * 카카오톡 공유용 이미지 리사이즈 함수 (Pica 사용)
 * - 카카오톡은 AVIF를 지원하지 않으므로 JPEG로 변환
 * - Pica Lanczos3 알고리즘으로 고품질 리사이징
 * - 가로 기준으로 리사이즈 (비율 유지)
 */
async function resizeImageForKakao(
    file: File,
    ...
    return new Promise((resolve, reject) => {
        ...
        img.onload = async () => {
            try {
                ...
                // 가로가 maxWidth 초과하는 경우에만 리사이즈 (비율 유지)
                if (width > maxWidth) {
                    height = Math.round((height / width) * maxWidth)
                    width = maxWidth
                    console.log(
                        `[카카오 리사이즈] ${originalWidth}x${originalHeight} → ${width}x${height}`
                    )
                }
                ...
                // Pica로 고품질 리사이즈 (또는 Canvas 폴백)
                if (pica && width !== originalWidth) {
                    ...
                    console.log("[카카오 리사이즈] Pica Lanczos3 완료")
                } else {
                    ...
                    if (width !== originalWidth) {
                        console.log("[카카오 리사이즈] Canvas 폴백 완료")
                    }
                }

                // JPEG 100% 품질로 변환 (카카오톡은 AVIF 미지원)
                const blob = await new Promise<Blob | null>((res) =>
                    destCanvas.toBlob(res, "image/jpeg", 1.0)
                )
                ...
                console.error("[카카오 리사이즈 에러]", error)
                reject(error)
            }
        }
...
/**
 * 클라이언트 측 이미지 리사이즈 함수 (Pica 라이브러리 사용)
 * - Pica: Lanczos3 알고리즘으로 고품질 리사이징 (Canvas보다 훨씬 선명)
 * - Cloudflare Worker가 최종적으로 고품질 AVIF 변환을 수행하므로
 * - 클라이언트에서는 해상도만 줄이고 품질은 그대로 유지
 * - 목표: 10MB 이하로만 압축하여 Cloudflare에서 고품질 압축되도록 함
 */
async function compressImageClient(
    file: File,
    maxSizeBytes: number = WORKER_MAX_SIZE,
    ...
    const originalSizeMB = (file.size / 1024 / 1024).toFixed(1)
    console.log(
        `[압축 시작] 원본 크기: ${originalSizeMB}MB, 목표: ${(maxSizeBytes / 1024 / 1024).toFixed(1)}MB 이하`
    )
    onProgress?.(`이미지 리사이즈 중... (${originalSizeMB}MB → 10MB 이하)`)
    ...
    return new Promise((resolve, reject) => {
        ...
        img.onload = async () => {
            try {
                ...
                const originalHeight = img.height
                console.log(
                    `[압축] 원본 해상도: ${originalWidth}x${originalHeight}`
                )

                // 가로 1200px 이하로만 리사이즈, 품질 100% 유지
                const maxWidth = 1200

                let width = originalWidth
                let height = originalHeight

                // 가로가 1200px 초과하는 경우에만 리사이즈 (비율 유지)
                if (width > maxWidth) {
                    height = Math.round((height / width) * maxWidth)
                    width = maxWidth
                    console.log(
                        `[압축] 해상도 조절: ${originalWidth}x${originalHeight} → ${width}x${height}`
                    )
                }
                ...
                onProgress?.(
                    `리사이즈 중... (${width}x${height}, Pica Lanczos3)`
                )

                // Pica로 고품질 리사이즈 (또는 Canvas 폴백)
                if (pica && width !== originalWidth) {
                    // Pica 사용 (Lanczos3 알고리즘)
                    await pica.resize(srcCanvas, destCanvas, {
                        quality: 3, // 최고 품질 (Lanczos3)
                        alpha: true,
                        unsharpAmount: 80,
                        unsharpRadius: 0.6,
                        unsharpThreshold: 2,
                    })
                    console.log("[Pica] 고품질 리사이즈 완료")
                } else {
                    // Canvas API 폴백 (리사이즈 필요 없거나 Pica 로드 실패)
                    const destCtx = destCanvas.getContext("2d")
                    if (!destCtx)
                        throw new Error("Canvas 컨텍스트를 생성할 수 없습니다.")
                    destCtx.drawImage(srcCanvas, 0, 0, width, height)
                    if (width !== originalWidth) {
                        console.log("[Canvas] 폴백 리사이즈 완료")
                    }
                }
                ...
                if (blob && blob.size <= maxSizeBytes) {
                    ...
                    console.log(
                        `[압축 완료] ${originalSizeMB}MB → ${(compressedFile.size / 1024 / 1024).toFixed(2)}MB (${width}x${height}, 품질: 100%)`
                    )
                    onProgress?.(
                        `압축 완료! (${(compressedFile.size / 1024 / 1024).toFixed(1)}MB)`
                    )
                    resolve(compressedFile)
                    return
                }

                // 100% 품질로도 10MB 초과하면 에러
                console.error(
                    `[압축 실패] ${width}x${height} + 100% 품질로도 ${maxSizeBytes / 1024 / 1024}MB 이하로 줄일 수 없음`
                )
                reject(
                    new Error(
                        `사진 파일이 너무 큽니다. 5MB 이하의 사진을 권장합니다.`
                    )
                )
            } catch (error) {
        ...
        img.onerror = () => {
            reject(
                new Error(
                    "이미지를 로드할 수 없습니다. 파일이 손상되었거나 지원하지 않는 형식일 수 있습니다."
                )
            )
        }

        // 파일을 Data URL로 변환하여 로드
        const reader = new FileReader()
        reader.onload = (e) => {
            img.src = e.target?.result as string
        }
...
function AdminMainContent(props: any) {
    ...
    // 업로드 결과 메시지 표시 여부 (X 버튼으로 닫기)
    const [showUploadResult, setShowUploadResult] = useState(false)
    // 현재 파일의 업로드 진행률 (0-100)
    ...
    const handleReplaceImage = async (index: number, newFile: File) => {
        ...
        try {
            ...
            // 2. 이미지 업로드 (Cloudflare Worker)
            const fileName = newFile.name || oldImage.filename
            let saved: any = null
            let uploadedUrl = ""
            let storagePath = ""

            console.log("이미지 업로드 시작... (Cloudflare Worker)")
            try {
                console.log("[Worker] REPLACE_START", {
                    name: newFile.name,
                    type: newFile.type,
                    size: newFile.size,
                })
                ...
                console.log("[Worker] PUBLIC_URL", uploadedUrl)

                storagePath =
                    key || extractStorageKeyFromUrl(uploadedUrl) || uploadedUrl
                ...
                console.error("Cloudflare 업로드 실패:", e)
                throw new Error(
            ...
            updatedImages[index] = {
                ...
                filename:
                    ...
                    storagePath ||
                    fileName,
                original_name: saved.original_name || newFile.name,
                public_url: saved.public_url || uploadedUrl,
                display_order: oldImage.display_order ?? index + 1,
                page_id: currentPageId as any,
                mime_type: saved.mime_type || newFile.type,
                file_size: saved.file_size || newFile.size,
            ...
            // 성공 알림
            alert(`이미지가 성공적으로 교체되었어요.: ${newFile.name}`)
            console.log(
                `이미지 교체 완료: ${oldImage.original_name} -> ${newFile.name}`
            )
        } catch (error) {
    ...
    const handleDeleteAllImages = async () => {
        ...
        try {
            setIsDeleting(true) // 삭제 중 상태 표시 (업로드와 분리)
    ...
    const handleLogin = useCallback(
        async (e: React.FormEvent) => {
            ...
            try {
                ...
                const assigned =
                    result.user &&
                    typeof result.user.page_id === "string" &&
                    result.user.page_id.trim().length > 0
                        ? result.user.page_id
                        : null
                ...
                loadAllPages()
                loadContactList(assigned ?? undefined)
                loadPageSettings(assigned ?? undefined)
            } catch (error) {
                console.warn("ID/PW 로그인 실패:", error)
                setLoginError(
                    "로그인에 실패했습니다. 카카오톡 채널 [로아크]로 문의해주세요."
                )
            } finally {
    ...
    const handlePhotoSectionImageUpload = async (
        ...
        try {
            ...
            if (existingImagePath || existingImageUrl) {
                try {
                    ...
                    const isR2Key = (
                        ...
                        if (!key) return false
                        // R2 키는 photos/, files/, images/, gallery/, audio/ 등으로 시작
                        return /^(photos|files|images|gallery|audio)\//.test(
                            key
                        )
                    }
                    ...
                    const isR2Url = (
                        ...
                        try {
                            const parsed = new URL(url)
                            // R2 도메인 확인 (예: pub-xxx.r2.dev 또는 커스텀 도메인)
                            return (
                                ...
                                parsed.pathname.startsWith("/audio/")
                            )
                        } catch {
                            return false
                        }
                    ...
                    // 1. existingImagePath가 있으면 우선 사용
                    if (existingImagePath) {
                        keyToDelete = existingImagePath
                        isR2Storage = isR2Key(keyToDelete)
                    }
                    // 2. existingImageUrl에서 키 추출
                    else if (existingImageUrl) {
                        isR2Storage = isR2Url(existingImageUrl)

                        if (isR2Storage) {
                            // R2 URL에서 키 추출
                            try {
                                const parsed = new URL(existingImageUrl)
                                keyToDelete = parsed.pathname.replace(
                                    /^\/+/,
                                    ""
                                )
                            } catch {
                                // URL 파싱 실패 시 extractStorageKeyFromUrl 사용
                                keyToDelete =
                                    extractStorageKeyFromUrl(existingImageUrl)
                            }
                        } else {
                            ...
                            if (idx !== -1) {
                                keyToDelete = existingImageUrl.substring(
                                    idx + marker.length
                                )
                            } else {
                                // fallback: 마지막 세그먼트만 있는 경우
                                keyToDelete = existingImageUrl
                                    .split("/")
                                    .slice(-2)
                                    .join("/")
                            }
                    ...
                    if (keyToDelete) {
                        console.log(
                            `기존 포토섹션 이미지 삭제 시도: ${keyToDelete} (R2: ${isR2Storage})`
                        )
            ...
            // 2. Cloudflare Worker 업로드 (AVIF 변환 - 갤러리용)
            let imageUrl = ""
            let imagePath = ""
            try {
                const { url, key } = await uploadToWorker(file, currentPageId)
                imageUrl = url
                imagePath = key || extractStorageKeyFromUrl(url) || ""
            } catch (e) {
                console.error("포토섹션 업로드 실패:", e)
                throw e
            }

            // 3. 카카오톡 공유용 JPEG 버전 업로드 (기존 kko_img가 없을 때만)
            // AVIF는 카카오톡에서 지원하지 않으므로 JPEG 버전 필요
            let kakaoImageUrl = pageSettings.kko_img || ""
            if (!kakaoImageUrl) {
                try {
                    console.log(
                        "[메인사진] 카카오톡 공유용 JPEG 버전 업로드 시작..."
                    )
                    const kakaoKey = `images/${encodeURIComponent(currentPageId)}/kakao_${Date.now()}.jpg`
                    const { publicUrl } = await uploadToR2(
                        file,
                        currentPageId,
                        kakaoKey
                    )
                    kakaoImageUrl = publicUrl
                    console.log(
                        "[메인사진] 카카오톡 공유용 이미지 업로드 완료:",
                        kakaoImageUrl
                    )
                } catch (kakaoError) {
                    console.warn(
                        "[메인사진] 카카오톡 공유용 이미지 업로드 실패 (메인 사진은 정상 업로드됨):",
                        kakaoError
                    )
                    // 카카오톡 이미지 업로드 실패해도 메인 사진은 정상 처리
                }
            }
            ...
            setPageSettings((prev: any) => ({
                ...
                ...(kakaoImageUrl && !prev.kko_img
                    ? { kko_img: kakaoImageUrl }
                    : {}),
            }))
            ...
            // 카카오톡 이미지가 새로 업로드되었고 기존에 없었으면 함께 저장
            if (kakaoImageUrl && !pageSettings.kko_img) {
                settingsToSave.kko_img = kakaoImageUrl
            }
            await savePageSettings(settingsToSave)

            console.log(
                "메인 사진이 업로드되었습니다." +
                    (kakaoImageUrl && !pageSettings.kko_img
                        ? " (카카오톡 공유용 이미지 자동 생성됨)"
                        : "")
            )
        } catch (error: unknown) {
    ...
    const handleFileSelect = async (
        ...
        setShowUploadResult(false) // 이전 결과 메시지 숨김
        ...
        try {
            ...
            for (let i = 0; i < totalFiles; i++) {
                ...
                try {
                    ...
                    const displayOrder =
                        existingImages.length + successCount + 1
                    console.log("[Worker] GALLERY_UPLOAD_START", {
                        name: file.name,
                        type: file.type,
                        size: file.size,
                        displayOrder,
                    ...
                    // 진행률 콜백을 전달하여 파일별 실시간 진행률 표시
                    const { url: uploadedUrl, key } = await uploadToWorker(
                        file,
                        currentPageId,
                        (fileProgress) => {
                    ...
                    const storagePath =
                        key ||
                        extractStorageKeyFromUrl(uploadedUrl) ||
                        uploadedUrl
                    ...
                    const newImg: ImageInfo = {
                        ...
                        display_order: saved.display_order || displayOrder,
                    }
            ...
            setCurrentUploadingFileName("")
            setShowUploadResult(true) // 결과 메시지 표시 (자동 사라지지 않음)

            setTimeout(() => {
                loadExistingImages()
            }, 200)
            loadAllPages()
            // setTimeout 제거: 사용자가 X 버튼으로 닫을 때까지 결과 메시지 유지
        } catch (error: unknown) {
    ...
    const handleKakaoImageChange = async (
        ...
        try {
            ...
            // Pica를 사용하여 고품질 리사이즈 (가로 1200px, JPEG 100%)
            // 카카오톡 공유 이미지는 JPEG 형식이어야 함 (AVIF 미지원)
            let processedFile = file
            const originalSizeMB = (file.size / 1024 / 1024).toFixed(1)

            console.log(
                `[카카오 이미지] 원본: ${file.name}, ${originalSizeMB}MB`
            )

            // 이미지 리사이즈 (Pica 사용)
            processedFile = await resizeImageForKakao(file, 1200)
            console.log(
                `[카카오 이미지] 리사이즈 완료: ${(processedFile.size / 1024 / 1024).toFixed(2)}MB`
            )
                ...
                >
                    ...
                    <div>
                        ...
                        >
                            ...
                            {/* 업로드 완료 메시지 (성공/실패 리포트) - X 버튼으로 닫기 */}
                            <AnimatePresence>
                                {!uploading &&
                                    showUploadResult &&
                                    (uploadSuccess > 0 ||
...
function InfoTab({
    ...
    >({})
    const uploadStatusTimers = React.useRef<
    ...
    const setTimedStatus = useCallback(
        (index: number, status: "success" | "error" | null) => {
            setUploadStatusMap((prev) => ({ ...prev, [index]: status }))
            const prevTimer = uploadStatusTimers.current[index]
            if (prevTimer) clearTimeout(prevTimer)

            if (status) {
                uploadStatusTimers.current[index] = setTimeout(() => {
                    setUploadStatusMap((prev) => ({ ...prev, [index]: null }))
                }, 3000)
            }
        },
        []
    ...
    const handleImageUpload = useCallback(
        async (index: number, file: File) => {
            ...
            setTimedStatus(index, null)

            try {
                const { url } = await uploadToWorker(file, pageId)
                setItems((prev) =>
                    prev.map((it, i) =>
                        i === index
                            ? {
                ...
                setTimedStatus(index, "success")
            } catch (error) {
                console.error("안내 이미지 업로드 실패:", error)
                setTimedStatus(index, "error")
            } finally {
                setUploadingMap((prev) => ({ ...prev, [index]: false }))
            }
        },
        [pageId, setTimedStatus]
    )
    ...
    const renderImageControls = useCallback(
        (index: number) => {
            const imageUrl = items[index]?.image
            const uploading = !!uploadingMap[index]
            const status = uploadStatusMap[index]
            ...
            return (
                ...
                >
                    <button
                        type="button"
                        onClick={() => triggerImageSelect(index)}
                        disabled={uploading}
                        style={{
                            padding: "4px 8px",
                            border: `1px solid ${theme.color.border}`,
        ...
        [items, triggerImageSelect, uploadStatusMap, uploadingMap]
    )

    React.useEffect(() => {
        return () => {
            Object.values(uploadStatusTimers.current).forEach((timer) => {
                clearTimeout(timer)
            })
        }
    }, [])
                ...
                <div>
                    {items.map((item, index) => (
                        <ItemCard
                            ...
                            renderLeftControls={renderImageControls}
                        />
                    ))}
                    <ButtonBase
                        onClick={addItem}
...
function TransportTab({
    ...
    async function applyAddressSelection(): Promise<void> {
        ...
        try {
            // 주소 검색 결과를 입력 칸에 자동으로 입력
            setVenue_address(chosenAddress.trim())
            setAddressInputUsed(true)
    ...
    }, [isAddressModalOpen, venue_address])

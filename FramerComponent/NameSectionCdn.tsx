...
interface NameSectionCdnProps {
    groomName: string
    brideName: string
    style?: React.CSSProperties
}

export default function NameSectionCdn(props: NameSectionCdnProps) {
    const { groomName, brideName, style } = props

    // ensure Pretendard upstream + P22 @font-face
    useEffect(() => {
        try {
            // 기본 ensure 호출 (외부 스크립트가 제공하는 API)
            typography?.ensure?.()
        } catch (_) {}
    }, [])
    ...
    const adjustTextSize = (): void => {
        if (!nameContainerRef.current || !groomRef.current || !brideRef.current) return

        const containerWidth = (nameContainerRef.current as HTMLDivElement).clientWidth
        const baseWidth = 390
        const minWidth = 280
        const maxWidth = 430
    ...
    return (
        ...
        >
            <motion.div
                ...
                transition={{ type: "spring", duration: 1, bounce: 0, delay: 0 }}
                style={{
                    // Use P22 + Pretendard stacks from external typography helper
                    fontFamily: '"P22 Late November", "Pretendard Variable", Pretendard, -apple-system, BlinkMacSystemFont, Segoe UI, Roboto, Helvetica, Arial, Apple SD Gothic Neo, Noto Sans KR, "Apple Color Emoji", "Segoe UI Emoji"',
            ...
            <motion.div
                ...
                initial={{ opacity: 0 }}
                animate={isInView ? { opacity: 1 } : { opacity: 0 }}
                transition={{ type: "spring", duration: 1, bounce: 0, delay: 1 }}
                style={{
                    fontFamily: '"P22 Late November", "Pretendard Variable", Pretendard, -apple-system, BlinkMacSystemFont, Segoe UI, Roboto, Helvetica, Arial, Apple SD Gothic Neo, Noto Sans KR, "Apple Color Emoji", "Segoe UI Emoji"',
                    fontSize: `${nameFontSize}px`,
                    textAlign: "center",
...
NameSectionCdn.defaultProps = {
    groomName: "GROOM",
    brideName: "BRIDE",
}

addPropertyControls(NameSectionCdn, {
    groomName: {

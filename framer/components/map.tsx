...
<stop offset="1" stop-color="#C8C8C8"/>
    ...
    // 네이버 지오코딩 (도로명/지번 주소 → 좌표)
    const searchWithNaverGeocode = async (query: string): Promise<any> => {
        return new Promise((resolve, reject) => {
            if (!window.naver || !window.naver.maps || !window.naver.maps.Service) {
                reject(new Error("Naver Maps Service가 로드되지 않았습니다"))
                return
            }
    ...
    // (기존) 구글 Places Text Search — 더 이상 사용하지 않음 (보존)
    ...
    const searchPlaceWithRetry = async (
        ...
        try {
            // 네이버 지오코딩 1순위 사용
            const result = await searchWithNaverGeocode(query)
            console.log("Naver Geocode 성공:", result)
        ...
        const fetchConfigAndVenue = async () => {
            try {
                ...
                if (roadAddress && roadAddress.trim()) {
                    // Property Controls의 도로명 주소가 최우선
                    finalVenueName = roadAddress.trim()
                    console.log("PC 도로명 주소 사용:", finalVenueName)
                } else if (forcePlaceName && forcePlaceName.trim()) {
                    // 강제 입력된 장소명이 있으면 우선 사용
                    finalVenueName = forcePlaceName.trim()
                    console.log("강제 입력된 장소명 사용:", finalVenueName)
                } else if (
                    settings?.venue_address &&
                    settings.venue_address.trim()
                ) {
    ...
    useEffect(() => {
        ...
        if (!window.naver || !window.naver.maps) {
            ...
            naverScript.src = `https://openapi.map.naver.com/openapi/v3/maps.js?ncpClientId=${currentNaverClientId}&submodules=geocoder`
...
addPropertyControls(NaverMapFix, {
    ...
    placeName: {
        ...
        defaultValue: "",
        placeholder:
            "검색할 장소의 이름 또는 주소 (강제 장소명이 없을 때 사용)",
    },
    roadAddress: {
        type: ControlType.String,
        title: "도로명 주소",

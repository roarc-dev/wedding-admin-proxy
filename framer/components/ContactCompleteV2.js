...
import {
  motion as importedMotion,
  AnimatePresence as ImportedAnimatePresence,
  MotionConfig,
} from "framer-motion";
...
const fallbackMotionConfig = ({ children }) => jsx(Fragment, { children });
...
const resolveMotionEnv = () => {
  ...
  const motionEnv = (framerNS && framerNS.motion) || g.motion || importedMotion || null;

  const motionDiv = motionEnv && typeof motionEnv.div === "function" ? motionEnv.div : fallbackMotion.div;
  const motionButton = motionEnv && typeof motionEnv.button === "function" ? motionEnv.button : fallbackMotion.button;
  const motionSvg = motionEnv && typeof motionEnv.svg === "function" ? motionEnv.svg : fallbackMotion.svg;

  const AnimatePresence =
    typeof (framerNS && framerNS.AnimatePresence) === "function"
      ? framerNS.AnimatePresence
      : ImportedAnimatePresence || fallbackAnimatePresence;

  const MotionConfigComponent =
    typeof (framerNS && framerNS.MotionConfig) === "function"
      ? framerNS.MotionConfig
      : MotionConfig || fallbackMotionConfig;
...
function ContactComplete(props) {
  ...
  const { motion, AnimatePresence, MotionConfig: MotionConfigComponent } = useMemo(resolveMotionEnv, []);
